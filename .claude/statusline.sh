...
# build_bar <pct> <width> <fill> <empty> <color> — only filled glyphs get <color>
build_bar() {
...
row=""
add "$(meter Ctx "$C_CTX" 10 "█" "░" "$ctx"  "")"
add "$(meter 5h  "$C_5H"   8 "▓" "▒" "$five" "$five_at")"
add "$(meter 7d  "$C_7D"   8 "▓" "▒" "$week" "$week_at")"
line2=$row
